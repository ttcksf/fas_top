...
//parallax
let para1 = document.querySelectorAll(".c-para1");
let ser1 = document.querySelector(".introduction");

window.addEventListener("scroll", function () {
  let mount = ser1.getBoundingClientRect().top;
  para1.forEach((tar) => {
    if (mount < 0) {
      tar.style.transform = `translateY(${
        mount * Number(tar.dataset.dir) * Number(tar.dataset.po)
      }px)`;
    } else {
...
let para2 = document.querySelectorAll(".c-para2");
let ser2 = document.querySelector(".service");

window.addEventListener("scroll", function () {
  let mount = ser2.getBoundingClientRect().top;
  para2.forEach((tar) => {
    if (mount < 0) {
      tar.style.transform = `translateY(${
        mount * Number(tar.dataset.dir) * Number(tar.dataset.po)
...
let para3 = document.querySelectorAll(".c-para3");
let ser3 = document.querySelector(".service__header--orange");

window.addEventListener("scroll", function () {
  let mount = ser3.getBoundingClientRect().top;
  para3.forEach((tar) => {
    if (mount < 0) {
      tar.style.transform = `translateY(${
        mount * Number(tar.dataset.dir) * Number(tar.dataset.po)
...
window.addEventListener("resize", function () {
  /* リサイズ時 */ clearTimeout(resizeTimerInterSect); /* タイマーキャンセル */
  resizeTimerInterSect = setTimeout(function () {
    startProcesses();
  }, 500);
});
// observer セットアップ
  ...
  entries.forEach((entry) => {
    if (entry.isIntersecting) {
      if (
        ...
        !entry.target.classList.contains("anim_act")
      ) {
        observe_fade(entry);
      }
    }
...
async function observe_fade(entry) {
  ...
  await new Promise((resolve) => {
    ...
    var transform = new DOMMatrix(
      getComputedStyle(entry.target).getPropertyValue("transform")
    );
    // x,yを指定の量加算 translateSelf:指定したベクトルを適用して行列を変更
    if (entry.target.classList.contains("fadeLR")) {
      transform.translateSelf(-fadeAm, 0);
    } else if (entry.target.classList.contains("fadeRL")) {
      transform.translateSelf(fadeAm, 0);
    } else if (entry.target.classList.contains("fadeTB")) {
      transform.translateSelf(0, -fadeAm);
    } else {
      transform.translateSelf(0, fadeAm);
    }
  ...
  if (fadeDuArr != null && !(fadeDuArr[0].indexOf("fadeDuRes") == -1)) {
    fadeDu =
      parseFloat(fadeDuArr[0].replace("fadeDuRes", "").split("_")[resPointer]) /
      10;
  ...
  if (fadeDeArr != null && !(fadeDeArr[0].indexOf("fadeDeRes") == -1)) {
    fadeDe =
      parseFloat(fadeDeArr[0].replace("fadeDeRes", "").split("_")[resPointer]) /
      10;
  ...
  if (fadeOpArr != null && !(fadeOpArr[0].indexOf("fadeOpRes") == -1)) {
    fadeOp =
      parseFloat(fadeOpArr[0].replace("fadeOpRes", "").split("_")[resPointer]) /
      10;
